<template>
  <div class="toggle-container flex-row align-items-center">
    <div v-if="!sidebar.opened" class="toggle-icon" @click="open">
...
<style scoped>
.toggle-container { background-color: #ffffff;height:48px;padding:10px;display: flex;flex-direction: row; }
.toggle-icon { flex:1;align-items: center;height: 22px;display: flex;cursor: pointer;padding-right: 10px;border-right: 1px solid #aaaaaa; }
.toggle-title { color: #666666;font-weight: bolder;flex: 0 0 auto;font-size: 19px;margin: 0 0 0 10px;letter-spacing: 5px;font-family: 'museo-1', 'museo-2', Verdana;text-shadow: #EEE 1px 1px 1px; }
</style>