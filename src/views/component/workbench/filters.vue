...
export default {
  ...
  methods: {
    task_list: function(taskUUID) {
      let self = this;
      let url = self.urls.team_task_list.format(self.team);
      http.post(url).then(function (response) {
        self.tasks = response.data;
        self.tasks_completed = true;
        if(self.tasks && self.tasks.length > 0) {
          if (self.$route.name === 'TaskInTeam') {
    ...
    select_task: function (uuid) {
      ...
      router.push({ name: 'TaskInTeam', params: { team: self.team, type: self.type, task: uuid } });
      self.task_get(uuid);
    },
    task_get: function (uuid) {
      let self = this;
      let url = self.urls.task_get.format(self.team, 'none', 'none', uuid);
